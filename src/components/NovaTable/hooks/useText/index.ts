...
export default function useText(this: any, item: ColumnItem) {
  ...
  const nearText = isString(unit) ? unit : Array.isArray(unit) ? unit[1] || "" : "";
  return {
    render: (scope: any) => {
      const { row } = scope;
      const values = props?.length ? props.map((prop) => getObjValue(prop, row)) : [getObjValue(prop, row)];

      if (!values || !values.length) return h("span", { class: "nova-table-text-view" }, emptyValue);
      ...
      const valueArr = valueRaws.map(({ label = "" }: any) => `${prefixText}${isSimpleArray(label) ? label.join(separator || ",") : label}${nearText}`);

      const value = cellClass === "column" ? valueArr.map((x, i) => h("div", { style: { color: color?.[i] } }, x)) : valueArr?.join(separator || "") || "- -";
